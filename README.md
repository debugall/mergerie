...
run triggered by a **schedule** that rewrote a note page. One coding session carries **three
iterations with their own diffs**, and the out-of-repo session carries its own, so the
per-follow-up diff is visible on both sides without an agent.
