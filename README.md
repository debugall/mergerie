...
- **Notes** — the sticky notes of everyday work, kept inside the tool: note pages in Markdown — with **sub-pages**, one level deep, so a general page can carry the detail of each of its points, and **Mermaid diagrams** rendered in place, the library shipped in the repository and loaded only when a page holds one — a prioritised todo list with due dates that double as **desktop reminders**, and a **morning brief** that opens the day — reminders, sessions waiting for an answer, failed verifications, fresh and dormant MRs, all computed locally with **no AI call**. `!214` and `PROJ-720` written in a note become links, and a merge request or a ticket can be added to the todos in one click.
