...
L'isolation réelle d'un lancement d'agent, séparée de la politique logique de permissions
(`agent/policy.js`) : ce module ne connaît que le job, ses chemins et le processus — jamais la
logique métier MR. Réglage `agent_sandbox` (Réglages → IA, poste, `'off'` par défaut) ; câblage
actuel limité aux lancements en LECTURE de `copilot.runReal` (review/explication/question) —
codage, vérification et plan restent sur le chemin direct (chantier suivant).

| Fichier | Rôle |
|---|---|
| `sandbox/spec.js` | **le contrat d'un job, pur** : `validerSpec` refuse toute forme inattendue (propriété inconnue, énumération hors liste, limite au-delà du plafond global) plutôt que de l'ignorer — `DEFAULTS`/`PLAFONDS` |
| `sandbox/policy.js` | compile la politique d'un MODE (review/verify/plan/edit) en `{permissions, limits}`, la hache (`hashPolicy`, stable, clés triées) et répond `can(mode, capacité)` — introspection pour l'API/l'UI, jamais l'application réelle de la permission |
| `sandbox/errors.js` | les codes d'erreur stables (`SANDBOX_UNAVAILABLE`, `SANDBOX_PATH_OUTSIDE_ROOT`, …), chacun traduit (`err.sandbox.*`, `public/i18n/erreurs.js`) |
| `sandbox/paths.js` | confinement de chemins : `resolveInside`/`assertNoExternalSymlink` (utilisés côté HÔTE, sur ce qu'un worktree a produit — les namespaces protègent déjà l'entrée, pas la collecte de sortie) et `listMountsFor` (la liste des montages d'un job, pure) |
| `sandbox/fs.js` | dossiers de job (`creerLayout`), extraction `git archive` **sans `.git`** dans la source (jamais un `git worktree add`, qui garderait un lien hors du dossier vers le clone partagé), calcul du patch d'un `plan`/`edit` par comparaison d'arbres (`diff -ruN`, aucun lien symbolique toléré dans la sortie), ménage (`nettoyerJob`, `gcJobs` au boot) |
| `sandbox/resources.js` | limites CPU/mémoire/PIDs : `systemd-run --user --scope` (cgroup délégué) quand disponible, repli `ulimit` sinon — marqué `degraded`, jamais présenté comme équivalent |
