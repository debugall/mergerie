...
Jira, Jenkins, Docker, l’export .docx, la veille de fond.
...
| `jobs/runners/` | un exécutant par sorte de job (`task`, `converge`, `local`, `ask`, `gitops`, `docker`, `reconcile`, `verify`, `review`, `merge-ai`), inscrit dans le registre au chargement par `jobs/index.js` |
...
- **Politique d'agent par saveur** (`agentpolicy.js`) : lecture (`review explain question modify explore ask test`) ⇒ `sansModeLarge(COPILOT_ARGS)` + `--restricted` (ou `--permission-mode default` + liste de lecture + `--setting-sources user`) — saveur inconnue ⇒ lecture, jamais écriture (fail-closed) ; écriture ⇒ trois modes de poste (`agent_write_mode`, Réglages → Session IA) : `sandbox` (défaut) une fois `agent_sandbox_verified` posé par le bouton **« Tester le sandbox »** (`agent/sandboxtest.js`, jamais un `PUT /api/config` direct) ⇒ `--settings` JSON (filesystem/réseau bornés par le CLI, Seatbelt macOS/bubblewrap Linux) ; `sandbox` non vérifié ou `allowlist` explicite ⇒ liste blanche de commandes (`allowlistEcriture`), sans sandbox ; `large` ⇒ ancien comportement, jamais le défaut d'un réglage mal lu (`data/config.js`). Copilot (`argvCopilot`) : `--deny-tool`/`--allow-tool` quand le binaire les connaît (sondé une fois, `capacites()`) — sinon écriture journalisée non restreinte, lecture REFUSÉE sauf `agent_read_unrestricted=1` explicite. Partout : `Read/Edit` interdits sur la base et le `.env`, `envAgent` en liste blanche (+ `MERGERIE_AGENT_ENV`), `--max-turns` par défaut et plafond de dépense du jour (`exigerBudget`), réglages de POSTE (`local_config`). Vérifié contre le CLI : `--allowedTools` est SANS effet sous `--dangerously-skip-permissions`, `--disallowedTools` tient.
