...
- **jira_watch** — `key` (clé du ticket, PK), `summary`, `status` + `status_category` (**dernier état connu** : c'est lui qu'on compare pour décider s'il y a eu changement), `added_at`, `checked_at`, `changed_at`, `error`. Alimente les notifications de changement d'état.
...
- **verification_run_cache** — le run **base** par jeu de SHAs (`repo_set_hash` → `run_json`). La base ne bouge pas pendant qu'on itère sur la branche ; la relancer coûterait plusieurs minutes pour un résultat connu.
...
**Ce qu'on a trouvé est restauré.** Worktrees supprimés dans un `finally`, orphelins ramassés au boot,
...
- Sessions : `GET/POST/PUT/DELETE /api/tasks[...]` (avec `kind`, `targets: [{repo_id, branch, base_branch}]`, `ask_questions` : opt-in « l'IA peut poser des questions », et **`session_id`** : session d'agent existante à reprendre au lieu d'en ouvrir une neuve — création seulement), `/:id/run|followup|md|clear-error|image`, `/:id/converge` (body `threshold`/`maxPasses` : du prompt à la/les MR convergée(s)).
