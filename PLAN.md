...
1. **Reviewer** les merge requests GitLab assisté par IA (skill `git-review` via un CLI d'agent).
...
- **Back** : Node 22.9+ + Express. Shelle `git` et le binaire d'agent (`copilot`/`claude`) via `child_process`. Client HTTP GitLab en `https`/`http` natif (agent TLS scopé). `gpt-tokenizer` (pur JS, hors-ligne, **optionnel** : repli sur estimation) pour le comptage de tokens du footer.
...
| `localcoder.js` | **« Codage hors dépôt »** : l'IA réalise le prompt dans des dossiers locaux arbitraires, en place, **sans git** (ni branche, ni commit, ni push) |
...
- **config** (row unique) — `gitlab_url`, `access_token`, `clone_path`, `review_skill`, `auto_refresh_minutes` (0 = désactivé), `language`, `review_explain` (`'1'`/`'0'` : générer l'explication pédagogique lors d'une review), `converge_threshold` (seuil cible /10, défaut 8) et `converge_max_passes` (plafond de passes, défaut 3), templates de prompt ; **Jira** : `jira_url`, `jira_email`, `jira_token` (secret masqué).
- **repo** — `project`, `url`, `branch_pattern` (vide = toutes les MR), `enabled`.
...
- **feed** — journal d'événements « frais » du footer : `type` (`mr_opened`/`mr_merged`), `mr_iid`, `project`, `author`, `title`, `at`. Émis par `discover` (nouvelle MR ; MR ouverte disparue de GitLab = mergée/fermée, avec garde d'ancienneté 7 j anti-fantômes + flag `mr.closed_seen` anti-doublon) et par le merge applicatif.
...
Optionnel, piloté par `config.auto_refresh_minutes` (0 = off, **minimum 1 min** contre les rate limits). **Côté serveur** : un `setInterval` relance `discoverAll()` (appel API GitLab), sans chevauchement (garde `autoRefreshBusy`) ; (re)configuré au démarrage et à chaque `PUT /api/config`. **Côté front** : l'intervalle est exposé dans `GET /api/status` (`autoRefreshMinutes`) ; un polling recharge alors les listes **depuis la base locale** (aucun appel GitLab supplémentaire) pour que la liste des MR à traiter se mette à jour toute seule.

## API REST (extrait)

- Config/dépôts : `GET/PUT /api/config`, `GET/POST/PUT/DELETE /api/repos[...]`, `POST /api/repos/bulk`, `GET /api/gitlab/projects|branches`.
...
- **Niveau MR** : `POST .../notes`.
- **Inline** (ligne précise) : `POST .../discussions` avec une `position` complète (`base_sha`/`start_sha`/`head_sha` récupérés via la MR, `old_path`/`new_path`, `old_line`/`new_line`). Les numéros de ligne sont calculés à partir du diff à contexte complet.
- **Réponses** : `POST .../discussions/:id/notes`.
- **Lecture** : `GET .../discussions` — inline affichés sous leur ligne, généraux dans le détail du rapport (notes système filtrées).

## Sécurité / robustesse


- **TLS scopé** à GitLab (jamais de `NODE_TLS_REJECT_UNAUTHORIZED` global) ; CA pinning ou insecure opt-in.
- **Token masqué** dans les logs ; stocké en BDD locale.
...
- **Contexte Jira** (donnée externe) : convertisseur ADF qui ne propage que les `href` http(s)/mailto ; fetch best-effort qui n'interrompt jamais le discover ; `jira_token` masqué comme le token GitLab.

## Interface

Sept onglets : **Reviews** (filtre segmenté sur les 3 stades d'une MR) · **Dev IA** (sous-onglets
Codage / Exploration) · **Statistiques** · **Git** (sous-onglets Actions / Navigation / **Commandes Git** / Explorateur de branches / Trouver une ref / Historique) · **Docker** (sous-onglets Compose / Hors-compose / Logs / Actions ; drift .env, badges santé du menu, tail live des logs, actions groupées) · **Jira** (mes tickets affectés : liste filtrable par statut — choix persisté — → détail avec contenu, métadonnées, commentaires, pièces jointes, changement d'état, ajout de commentaire) ·
**Réglages** (sous-onglets Règles / Dépôts / Notifications / Général / **Git** / **Jira** / Merge Request / AI sessions ; les champs sont éclatés sur plusieurs sous-onglets mais restent **un seul `#configForm`** via l'attribut HTML `form=` : URL GitLab / access token / dossier de clonage → **Git** ; URL/email/jeton Jira → **Jira** ; + la **palette de commandes** dans Git).
...
WSL/MAC/Linux. GitLab self-hosted avec CA d'entreprise → `GITLAB_CA_CERT` ou `GITLAB_INSECURE_TLS=1`, et `GIT_CLONE_SSH=1` pour le clone. Agent IA lancé avec le flag « yolo » pour autoriser la modification de fichiers. `.env` chargé automatiquement au démarrage.
