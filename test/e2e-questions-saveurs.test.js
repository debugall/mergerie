  ...
  test('la case est visible et enregistrée depuis l’écran, pour les trois saveurs', async (t) => {
    ...
    try {
      ...
      for (const kind of ['code', 'local', 'explore']) {
        ...
        /* La case vit SOUS LES YEUX, hors de l'accordéon « Avancé » (replié par défaut) : une
           décision qu'on prend à chaque session. Elle doit être PROPOSÉE dans les trois saveurs. */
        ...
        assert.equal(await page.locator('#taskAdvanced').evaluate((e) => e.open), false,
          `l’avancé démarre replié en « ${kind} »`);
