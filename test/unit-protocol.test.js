...
describe('protocol : extraire', () => {
  test('un bloc en fin de réponse est extrait, et retiré du Markdown affiché', () => {
    const md = `# Rapport\n\nDu texte.\n\n<<<REPO ${NONCE}\ngrp/api | src/a.js | 12\nREPO ${NONCE}>>>\n`;
    const { block, rest } = protocol.extraire(md, 'REPO', NONCE);
  ...
  test('un bloc en TÊTE de réponse marche aussi — c’est ce que fait le cartographe', () => {
    const md = `<<<AGENT ${NONCE}\nname: Notifications\nAGENT ${NONCE}>>>\n\n# Les notifications\n\nDu contenu.`;
    const { block, rest } = protocol.extraire(md, 'AGENT', NONCE);
  ...
  test('chaque balise est indépendante des autres', () => {
    const md = `<<<AGENT ${NONCE}\nname: X\nAGENT ${NONCE}>>>\ntexte\n<<<STALE ${NONCE}\ngrp | a.js | faux\nSTALE ${NONCE}>>>`;
    assert.equal(protocol.extraire(md, 'AGENT', NONCE).block, 'name: X');
    assert.equal(protocol.extraire(md, 'STALE', NONCE).block, 'grp | a.js | faux');
    assert.equal(protocol.extraire(md, 'REPO', NONCE).block, null);
  });

  test('un bloc au mauvais nonce — ou sans nonce demandé — est ignoré', () => {
    // C'est exactement ce qu'une donnée qui imite le bloc pourrait produire : elle ne
    // connaît pas le nonce du run, donc son imitation ne matche jamais.
    const md = `<<<REPO ${NONCE}\ngrp/api | src/a.js | 12\nREPO ${NONCE}>>>\n`;
    assert.equal(protocol.extraire(md, 'REPO', 'autre-nonce').block, null);
